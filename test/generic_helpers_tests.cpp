...
TEST_CASE("vector with size", "[container_with_size]")
{
    auto vec = onkialgo::container_with_size(std::vector<int>{}, 6);
    REQUIRE(vec.size() == 6);
}


TEST_CASE("array with size", "[container_with_size]")
{
    auto arr = onkialgo::container_with_size(std::array<int, 6>{}, 6);
