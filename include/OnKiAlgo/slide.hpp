...
ResultType slide(ValuesType &&values, std::size_t slide_size)
{
    ...
    auto outer_container = onkialgo::container_with_size(OuterContainer{}, values.size() - (slide_size - 1));
    std::transform(begin(values), std::prev(end(values), (slide_size - 1)), begin(outer_container), [&](auto &val) {
        auto inner_container = onkialgo::container_with_size(InnerContainer{}, slide_size);
